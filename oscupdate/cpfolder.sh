...
read folder2cop
echo please enter the FOLDER  Destination
read folderDes
...
do 
	:
		...
		if [[ $pyy == 2 ]]  && [[ ! "${a,,}" =~ "python3" ]] 
		then
			if [ -d $direc$i$folderDes ] 
			then
			cp -Rf $folder2cop$file2cop $direc$i$folderDes
			echo $file2cop "copied to " $i
			else
			echo $i "does not contain destination"
			echo "we will create foler " $folderDes
			mkdir $direc$i$folderDes
			cp -Rf $folder2cop$file2cop $direc$i$folderDes
			echo $file2cop "copied to " $i
			fi
...
do
	...
	if [[ $pyy == 2 ]]  
	then
	if [ -d "/media/oscam_fold/"$ii$folderDes ] 
	then
	cp -Rf $folder2cop$file2cop "/media/oscam_fold/"$ii$folderDes
...
do 
	:
		...
		then
			...
			echo "we will create foler " $folderDes
			mkdir $direc$i$folderDes
			cp $folder2cop$file2cop $direc$i$folderDes
			echo $file2cop "copied to " $i
			fi
		fi
done
