...
a=requests.get('https://cccam.ch/iptv-free/')
aa=a.text.encode('ascii', 'ignore').strip('\r')
lim=re.findall('href="(http://test.cccam.ch.*?)"',aa,re.DOTALL)[0].replace('amp;','')
f = requests.get(lim)
ttx=f.text.encode('ascii', 'ignore').split('\r')
locatf='/tmp/iptvcccamch.m3u'
d=open(locatf,'w')
for i in ttx:
    d.write(i)
d.close()
...
for i in range(len(m3uf)):
    if m3uf[i].find('EXTINF')>-1:
        count=i
        break
if os.path.isfile('/etc/enigma2/userbouquet.baraem__tv_.tv'):
    os.remove('/etc/enigma2/userbouquet.baraem__tv_.tv')
    file=open('/etc/enigma2/userbouquet.baraem__tv_.tv','w')
    file.close()
file=open('/etc/enigma2/userbouquet.baraem__tv_.tv','w')
file.write('#NAME baraem (TV)\n')
while i <len(m3uf)-1:
...
for i in range(len(m3uf)):
    if m3uf[i].find('EXTINF')>-1:
        count=i
        break
kodi=open('/tmp/iptvccamch.xml','w')
