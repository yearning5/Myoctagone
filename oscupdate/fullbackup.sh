...
getMachineName="$(awk '/^getMachineName/ { print $0}' "$Boxbranding_File" | cut -d "=" -f 2 | cut -d "/" -f 1)"
getImageFileSystem="$(awk '/^getImageFileSystem/ { print $0}' "$Boxbranding_File" | cut -d "=" -f 2 | cut -d "/" -f 1)"
MTDKERNEL=$getMachineMtdKernel
MTDROOTFS=$getMachineMtdRoot
KERNELBIN=$getMachineKernelFile
ROOTFSBIN=$getMachineRootFile
ROOTFSTYPE=$(echo $getImageFileSystem | tr -d "_")
...
echo "BOX MACHINEBUILD = >"$MACHINEBUILD"<"
echo "BOX MACHINENAME = >"$GetBoxName"<"
echo "BOX MACHINEBRAND = >"$Octagon"<"
echo "BOX MODEL = >" sf8008 "<"
echo "OEM MODEL = >" octagon "<"
echo "IMAGEFOLDER = >"$IMAGEFOLDER"<"
echo "UBINIZE = >""<"
echo "MKUBIFS = >""<"
echo "MTDBOOT = >"none"<"
echo "MTDKERNEL = >"$MTDKERNEL"<"
echo "MTDROOTFS = >"$MTDROOTFS"<"
echo "ROOTFSBIN = >"$ROOTFSBIN"<"
echo "KERNELBIN = >"$ROOTFSBIN"<"
#echo "ROOTFSSUBDIR = >"linuxrootfs4"<"
echo "ROOTFSTYPE = >"$ROOTFSTYPE"<"
echo "EMMCIMG = >" usb_update.bin "<"
echo "IMAGEDISTRO = >"$IMAGEDISTRO"<"
echo "DISTROVERSION = >"$DISTROVERSION"<"
echo "MTDBOOT = >"none"<"
echo "USB RECOVERY = >"False"<"
...
echo "rename this file to "" force "" to force an update without confirmation" > $MAINDESTROOT/unforce_"$MACHINEBUILD.txt"
...
echo "$START_DATE $END_DATE" | awk ' { printf("It took %.2f Minutes to complete this job.\n", ($2-$1)); } ' >> /tmp/FullBackup.txt
a=$(stat -c%s $DIRECTORY/$IMAGEDISTRO-$DISTROVERSION-$MODEL-backup-"$DATE"_mmc.zip)
echo "File size is : " $((a/1000000)) "Mb" >> /tmp/FullBackup.txt
