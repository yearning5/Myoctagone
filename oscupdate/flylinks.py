...
Agent = {'User-agent': 'Mozilla/5.0 (X11; U; Linux x86_64; de; rv:1.9.0.15) Gecko/2009102815 Ubuntu/9.04 (jaunty) Firefox/3.',
 'Connection': 'Close'}
def f77(seq):
    a=list(dict.fromkeys(seq))
    return a
http = urllib3.PoolManager()
link='https://flylinks.net/'
...
links=[]
for i in range(n):
    links.append(hos[i]+'/get.php?username='+user[i]+'&password='+pas[i]+'&type=m3u&output=ts')
    ...
    try:
        ...
        for line in aa:
            if line.strip():
                if '#EXTVLCOPT' not in line:
                    m3uf.append(line)
        tmmp='/tmp/'+'flylinks'+str(random.randrange(1000,9999))+'.m3u'
        fid=open(tmmp,'w')
        for l in m3uf:
            fid.write(l)
        fid.close()
        ...
        for i in range(len(m3uf)):
            if m3uf[i].find('EXTINF')>-1:
                count=i
                break
        if os.path.isfile('/etc/enigma2/userbouquet.flylinks__tv_.tv'):
            ax='flylinks'+str(random.randrange(1000,9999))
