...
if 'pkt' in image.lower():
    source='/var/keys/'+filee

elif 'vix' in image.lower():
    source='/etc/tuxbox/config/'+filee
else:
    source='/etc/tuxbox/oscam-emu/'+filee
...
for i in images:
    ...
    if 'pkt' in i.lower() or 'hyp' in i.lower():
        ...
        dest='/etc/tuxbox/config/'+filee
        #try: os.system('cp '+source+' '+path+i+dest)
        #except: pass
    else:
        dest='/etc/tuxbox/oscam-emu/'+filee
        #try: os.system('cp '+source+' '+path+i+dest)
        #except: pass
    try:
...
os.popen('mount /dev/mmcblk0p16 /media/oscam_fold')
...
for n in imgs:
    try:
        ...
        if 'pkt' in image.lower():
            os.system('cp '+source+' '+osc_fold+n+'/var/keys/'+filee)
            print osc_fold+n
        elif 'vix' in image.lower():
            os.system('cp '+source+' '+osc_fold+n+'/etc/tuxbox/config/'+filee)
            print osc_fold+n
        elif 'define' in image.lower():
            os.system('cp '+source+' '+osc_fold+n+'/home/gx/local/oscam/'+filee)
            print osc_fold+n
        else:
            os.system('cp '+source+' '+osc_fold+n+'/etc/tuxbox/oscam-emu/'+filee)
            print osc_fold+n
    except:
        pass
